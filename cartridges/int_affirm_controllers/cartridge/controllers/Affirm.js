...
function applyDiscount() {
    ...
    if (request.httpMethod == 'OPTIONS') {
        ...
        var cart = app.getModel('Cart').get();
        var basket = BasketMgr.getCurrentOrNewBasket();
        var status = cart.addCoupon(affirmDataOrder.discount_code);
        var discount;
        var validStatus = false;
        if (status.CouponStatus === 'APPLIED') {
            var affirmShippingOptions = affirm.utils.getShippingOptions();
            discount = basket.couponLineItems[0].priceAdjustments[0].priceValue * -100;	
            validStatus = true;
        } else {
            discount = 0;
        }
        ...
        var responseObject =  JSON.stringify({
            ...
            "merchant_internal_order_id": basket.UUID,
            "shipping_options": affirmShippingOptions,
            "tax_amount": basket.totalTax.value * 100,
            "total_amount":  basket.totalGrossPrice.value * 100
        });
        prepareResponse();
        response.writer.print(responseObject);
    }
