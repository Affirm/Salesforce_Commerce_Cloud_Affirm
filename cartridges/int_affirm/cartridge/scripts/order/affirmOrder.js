    ...
    var affirmOrder = function () {
        ...
        this.updateAttributes = function (
            ...
            try {
                paymentInstrument.paymentTransaction.transactionID =
                    reponse.events[0].id;
                paymentInstrument.paymentTransaction.amount = new Money(
                    reponse.amount,
                    order.currencyCode
                ).divide(100);
            ...
            OrderMgr.processOrders(
                function (order) {
                    try {
                        ...
                        logger.error(
                            "Affirm. File - affirmOrder. Error - {0}",
                            e
                        );
                    }
                },
                "(status = {0} OR status = {1}) AND custom.AffirmPaymentAction = {2} AND custom.AffirmStatus = {3}",
        ...
        this.voidOrders = function () {
            OrderMgr.processOrders(
                ...
                "status = {0} AND custom.AffirmStatus = {1}",
        ...
        this.updateOrders = function () {
            ...
            OrderMgr.processOrders(
                function (order) {
                    try {
                        ...
                        if (
                            ...
                            orderShipment.shippingMethod.displayName
                        ) {
                            updateObject.shipping_carrier =
                                orderShipment.shippingMethod.displayName;
                        }
                        if (orderShipment.trackingNumber) {
                            updateObject.shipping_confirmation =
                ...
                "custom.AffirmStatus != NULL AND lastModified > {0}",
                lastUpdateDateTime.toISOString()
            );
        ...
        this.trackOrderConfirmed = function (order) {
            ...
            if (order) {
                orderInfo = {
                    ...
                    paymentMethod: order.getPaymentInstruments()[0]
                        .paymentMethod,
                    revenue: priceToInteger(
                        order.getMerchandizeTotalNetPrice()
                    ),
                    shipping: priceToInteger(order.getShippingTotalPrice()),
                    shippingMethod:
                        order.getShipments().length === 1 &&
                        order.getShipments()[0].shippingMethod
                            ? order.getShipments()[0].shippingMethod.displayName
                            : "other",
                ...
                if (!lineItem.bundledProductLineItem) {
                    ...
                    if (lineItem.product) {
                        ...
                        if (lineItem.product.primaryCategory) {
                            ...
                            obj.category =
                                lineItem.product.classificationCategory.ID;
                        }
                    }
